...
const SCENARIOS = {
  gabiao: {
    ...
    beforeImg: 'servico-terraplanagem.jpg',
    afterImg: 'servico-muro-gabiao.jpg'
  },
  loteamento: {
    ...
    beforeImg: 'servico-terraplanagem.jpg',
    afterImg: 'servico-loteamento.jpg'
  },
  piscina: {
    ...
    beforeImg: 'servico-fundacoes.jpg',
    afterImg: 'obra-piscina-arrimo-jm.jpg'
  }
};
...
function initBeforeAfterSlider() {
  ...
  const scenarioBtns = document.querySelectorAll('.scenario-pill:not(.modal-scenario-pill)');
  ...
  const updateSliderPosition = (val) => {
    ...
    beforeContainer.style.webkitClipPath = `inset(0 ${100 - val}% 0 0)`;
...
/* ==========================================================================
   MODAL POPUP EXCLUSIVO: ANTES & DEPOIS INTERATIVO COM ARRASTE EM TELA CHEIA
   ========================================================================== */
function initBeforeAfterModal() {
  const overlay = document.getElementById('beforeAfterModalOverlay');
  const closeBtn = document.getElementById('modalCloseBeforeAfterBtn');
  const dismissBtn = document.getElementById('modalDismissBeforeAfterBtn');
  const rangeInput = document.getElementById('modalComparisonRange');
  const beforeContainer = document.getElementById('modalComparisonBefore');
  const sliderHandle = document.getElementById('modalSliderHandle');
  const scenarioBtns = document.querySelectorAll('.modal-scenario-pill');
  const imgBefore = document.getElementById('modalImgBefore');
  const imgAfter = document.getElementById('modalImgAfter');
  const captionTitle = document.getElementById('modalScenarioTitle');
  const captionDesc = document.getElementById('modalScenarioDesc');
  ...
  if (rangeInput && beforeContainer && sliderHandle) {
    const updateModalSlider = (val) => {
      beforeContainer.style.clipPath = `inset(0 ${100 - val}% 0 0)`;
      beforeContainer.style.webkitClipPath = `inset(0 ${100 - val}% 0 0)`;
      sliderHandle.style.left = `${val}%`;
    };
    ...
    rangeInput.addEventListener('input', (e) => {
      updateModalSlider(e.target.value);
    });
    ...
    scenarioBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const key = btn.getAttribute('data-modal-scenario');
        const data = SCENARIOS[key];
        if (data && imgBefore && imgAfter) {
          imgBefore.src = data.beforeImg;
          imgAfter.src = data.afterImg;
          if (captionTitle) captionTitle.textContent = data.title;
          if (captionDesc) captionDesc.textContent = data.desc;
          rangeInput.value = 50;
          updateModalSlider(50);
        }
      });
    });
...
window.openBeforeAfterModal = function(scenarioKey) {
  const overlay = document.getElementById('beforeAfterModalOverlay');
  if (!overlay) return;

  const key = scenarioKey || 'gabiao';
  const data = SCENARIOS[key];
  ...
  if (data) {
    ...
    if (rangeInput && beforeContainer && sliderHandle) {
      ...
      beforeContainer.style.webkitClipPath = 'inset(0 50% 0 0)';
      sliderHandle.style.left = '50%';
    }
  }

  document.querySelectorAll('.modal-scenario-pill').forEach(btn => {
    btn.classList.toggle('active', btn.getAttribute('data-modal-scenario') === key);
  });
